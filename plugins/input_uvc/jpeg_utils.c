...
//int jpeg_enc_yv12(unsigned char* buffer, int width, int height, unsigned char *pJPEGbuff, int szJPEGbuff, int quality, char* filename)
//int jpeg_enc_yv12(struct vdIn *vd, unsigned char *pJPEGbuff, int szJPEGbuff, int quality)
int compress_yuyv_to_jpeg(struct vdIn *vd, unsigned char *pJPEGbuff, int szJPEGbuff, int quality)
{
    struct jpeg_compress_struct cinfo;
    struct jpeg_error_mgr jerr;
	...
	unsigned char* buffer = vd->framebuffer;
	int width  = vd->width;
 ...
 //   jpeg_stdio_dest(&cinfo, outfile);
	dest_buffer(&cinfo, pJPEGbuff, szJPEGbuff, &written);
