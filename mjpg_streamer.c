...
DBG("starting to open output plugin(s)\n");
    /* open output plugin */
    for(i = 0; i < global.outcnt; i++) {
 DBG("i=%d\n",i);
...
DBG("global.out[i].plugin=%d\n",global.out[i].plugin);
        global.out[i].handle = dlopen(global.out[i].plugin, RTLD_LAZY);
        if(!global.out[i].handle) {
            DBG("ERROR: could not find output plugin %s\n", global.out[i].plugin);
            DBG("       Perhaps you want to adjust the search path with:\n");
            DBG("       # export LD_LIBRARY_PATH=/path/to/plugin/folder\n");
            DBG("       dlopen: %s\n", dlerror());
            closelog();
            exit(EXIT_FAILURE);
        }
...
DBG("output_init completed\n");		
        global.out[i].stop = dlsym(global.out[i].handle, "output_stop");
        if(global.out[i].stop == NULL) {
            DBG("%s\n", dlerror());
            exit(EXIT_FAILURE);
        }
DBG("output_stop completed\n");		
        global.out[i].run = dlsym(global.out[i].handle, "output_run");
        if(global.out[i].run == NULL) {
            DBG("%s\n", dlerror());
            exit(EXIT_FAILURE);
        }
DBG("try to find optional command\n");
        /* try to find optional command */
